...
function alphaBeta(
  ...
  if (maximazingPlayer) {
    ...
    for (const validColumn of validColumns) {
      ...
      const { score: newScore } = alphaBeta(connectFourBoardCopy, depth - 1, true, alpha, beta)
      if (newScore < score) {
        score = newScore
        bestColumn = validColumn
      }
...
export const getMove = async (board: number[][], depth: number) => {
  const connectFourBoard = new ConnectFourBoard(BoardConfig.AGENT, false, board)
  const result = alphaBeta(connectFourBoard, depth, true, -INF, INF)
